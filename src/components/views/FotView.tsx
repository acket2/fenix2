...
import { getFotRecords, updateFotRecord, deleteFotRecord } from '../../utils/storage';
    ...
    for (let i = day - 6; i <= day; i++) {
      if (i > 0) {
        ...
        if (status === 'full' || status === 'half') {
          const dayRate = (record.dayAmounts && record.dayAmounts[i] !== undefined) ? record.dayAmounts[i] : rate;
          weekEarned += dayRate;
        }
      }
    }
          ...
          <table className="w-full text-left border-collapse" style={{ minWidth: `${300 + daysInMonth * 40}px` }}>
            ...
            <tbody>
              {records.map((record) => {
                ...
                return (
                  <tr key={record.id} className="border-b border-slate-700/30 hover:bg-slate-900/50 transition-colors group">
                    <td className="p-2 sm:sticky sm:left-0 bg-slate-800 text-white group-hover:bg-slate-900/50 border-r border-slate-700/50 sm:z-10" style={{ minWidth: '160px', maxWidth: '160px' }}>
                      <div className="flex items-center gap-1 w-full relative">
                        {user.role === 'admin' && (
                          <button 
                            onClick={() => {
                              if (window.confirm(`Удалить сотрудника "${record.workerName}"?`)) {
                                deleteFotRecord(record.id);
                                loadData();
                              }
                            }}
                            className="absolute -left-1 opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-400 bg-slate-800 rounded p-1 transition-opacity z-20"
                            title="Удалить сотрудника"
                          >
                            <Trash2 className="w-3 h-3" />
                          </button>
                        )}
                        <div className={`flex-1 min-w-0 ${user.role === 'admin' ? 'ml-5' : ''}`}>
                          {user.role === 'admin' ? (
                            <DebouncedInput 
                              value={record.workerName} 
                              onChange={(val) => handleNameChange(record, val)}
                              className="w-full text-xs font-bold text-white bg-transparent border-b border-transparent hover:border-slate-300 focus:border-blue-500 outline-none truncate"
                            />
                          ) : (
